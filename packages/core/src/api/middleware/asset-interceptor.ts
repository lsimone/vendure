...
export class AssetInterceptor implements NestInterceptor {
    ...
    constructor(private configService: ConfigService, private requestContextService: RequestContextService) {
    ...
    intercept<T = any>(context: ExecutionContext, next: CallHandler<T>): Observable<T> {
        ...
        if (toAbsoluteUrl === undefined || !this.requestContextService.isGraphQl(context)) {
            return next.handle();
        }
